...
namespace MicroOrm.Pocos.SqlGenerator
{
    ...
    public class SqlGenerator<TEntity> : ISqlGenerator<TEntity> where TEntity : new()
    {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual string GetSelectAllPaged(string sinceId, string count)
        {
            return this.GetSelectPaged(new { },sinceId,count);
        }
        ...
        /// <returns></returns>
        public virtual string GetSelect(object filters, string filterOperator = "AND")
        {
            //Projection function
            ...
            if (containsFilter)
                sqlBuilder.AppendFormat(" WHERE {0} ", this.ToWhere(filterProperties,filterOperator));

            //Evaluates if this repository implements logical delete
            if (this.LogicalDelete)
            {
                if (containsFilter)
                    sqlBuilder.AppendFormat( filterOperator + " [{0}].[{1}] != {2}",
                                            this.TableName,
        ...
        /// <param name="filters"></param>
        /// <returns></returns>
        public virtual string GetSelectPaged(object filters, string sinceId, string count, string filterOperator = "AND")
        {
            //Projection function
            Func<PropertyMetadata, string> projectionFunction = (p) =>
            {
            ...
            var sqlBuilder = new StringBuilder();
            sqlBuilder.AppendFormat("SELECT TOP "+ count +" {0} FROM [{1}].[{2}] WITH (NOLOCK)",
                                    string.Join(", ", this.BaseProperties.Select(projectionFunction)),
                                    this.Scheme,
                                    this.TableName);
            ...
            if (containsFilter)
            {
                ...
                sqlBuilder.AppendFormat(filterOperator + " ID > " + sinceId);
            }
            else
            {
                sqlBuilder.AppendFormat(" WHERE ID > " + sinceId);
            }
        ...
        /// <returns></returns>
        private string ToWhere(IEnumerable<string> properties, string filterOperator="AND")
        {
            return string.Join(" " + filterOperator , properties.Select(p => {
